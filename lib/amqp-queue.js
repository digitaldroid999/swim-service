...
/**
 * AMQP 1.0 consumer (jumpstart-latest/node-consumer pattern): higher credit than 100 so backlog
 * is not stuck on the broker behind a small unsettled window; accept (settle) once the body is
 * copied then run async onMessage on a serialized chain. Trade-off: no redelivery after accept.
 *
 * @param {object} opts
...
function connectAmqpQueue(opts) {
  ...
  /** Serialize handlers like jumpstart's synchronous onMessage (no overlapping work). */
  ...
  function tryConnect() {
    ...
    const creditRaw = parseInt(process.env.SWIM_AMQP_CREDIT_WINDOW || '1000', 10);
    const creditWindow = Number.isFinite(creditRaw) ? Math.min(65535, Math.max(10, creditRaw)) : 1000;
    ...
    async function processOneAmqpMessage(ctx) {
      const xmlStr = messageBodyToString(ctx.message);
      if (!xmlStr) {
        try {
          ctx.delivery.accept();
        } catch {
      ...
      try {
        ...
        console.warn(
          `${logPrefix} message processing error (payload already accepted):`,
          err.message
        );
      }
    }
    ...
    container.on('message', (ctx) => {
      inboundTail = inboundTail
        .then(() => processOneAmqpMessage(ctx))
        .catch(() => {
          /* errors logged inside processOneAmqpMessage */
        });
    });
