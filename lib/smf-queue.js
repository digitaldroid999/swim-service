 ...
 *
 * Settlement follows jumpstart-latest semantics (Java SolaceConsumerV2: Session.AUTO_ACKNOWLEDGE
 * after synchronous body read + output; node-consumer: accept right after body read): we
 * CLIENT-ack as soon as the payload is copied into JS, then run async onMessage. That removes
 * messages from broker GM promptly so deep backlogs do not TTL-expire while parse/DB runs.
 * Trade-off: crash between ack and DB write loses the message (no redelivery).
 *
 * @param {object} opts
...
function connectSmfQueue(opts) {
  ...
  /** Serialize onMessage; broker ack happens inside handler right after body copy (jumpstart-style). */
  ...
  async function processOneInboundMessage(message) {
    inboundSeq += 1;
    const msgType = message.getType();
    ...
    try {
      ...
      console.warn(
        `${logPrefix} message processing error (payload already acked on broker):`,
        err.message
      );
    }
  }
  ...
  function connectInternal() {
    ...
    session.on(solace.SessionEventCode.UP_NOTICE, () => {
      ...
      messageConsumer.on(solace.MessageConsumerEventName.MESSAGE, (message) => {
        // Serialize onMessage (SQLite); ack already happened inside processOneInboundMessage
        // after body extract so broker GM does not hold messages during slow work.
        inboundTail = inboundTail
          .then(() => processOneInboundMessage(message))
          .catch(() => {
            /* errors logged inside processOneInboundMessage */
          });
      });
