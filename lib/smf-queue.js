...
function connectSmfQueue(opts) {
  ...
  /** One in-flight handler at a time so acks keep pace and the broker queue does not age out (TTL). */
  ...
  function connectInternal() {
    ...
    session.on(solace.SessionEventCode.UP_NOTICE, () => {
      ...
      messageConsumer.on(solace.MessageConsumerEventName.MESSAGE, (message) => {
        // Chain handlers: Solace may keep delivering while app acks are pending; overlapping
        // slow work (e.g. SQLite + parse) lets the broker backlog grow and messages can TTL-expire.
